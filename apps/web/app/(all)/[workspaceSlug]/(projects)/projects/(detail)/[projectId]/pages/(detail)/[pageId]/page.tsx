  ...
  if (pageDetailsError || !canCurrentUserAccessPage)
    return (
      <div className="flex h-full w-full flex-col items-center justify-center">
        <h3 className="text-center text-16 font-semibold">{t("pages_ui.not_found.title")}</h3>
        <p className="mt-3 text-center text-13 text-secondary">{t("pages_ui.not_found.description")}</p>
        <Link
          href={`/${workspaceSlug}/projects/${projectId}/pages`}
          className={cn(getButtonStyling("secondary", "base"), "mt-5")}
        >
          {t("pages_ui.not_found.view_others")}
        </Link>
      </div>
    );
