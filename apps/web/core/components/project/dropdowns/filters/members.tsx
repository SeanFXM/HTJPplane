...
export const FilterMembers = observer(function FilterMembers(props: Props) {
  ...
  return (
    <>
      <FilterHeader
        title={`${t("workspace_projects.filter_labels.members")}${appliedFiltersCount > 0 ? ` (${appliedFiltersCount})` : ""}`}
        isPreviewEnabled={previewEnabled}
        handleIsPreviewEnabled={() => setPreviewEnabled(!previewEnabled)}
      />
      {previewEnabled && (
        <div>
          <p className="mb-1.5 text-11 text-tertiary">{t("workspace_projects.filter_labels.members_help")}</p>
          {sortedOptions ? (
